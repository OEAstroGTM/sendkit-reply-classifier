...
import { safeBookingLink, safeAvailableTimes } from "./calendly.js";
...
Length: under 60 words, not counting the list of times. Most good replies are three or four short lines.
...
If they are saying no, not now, or already have someone:
- Do not offer a call. Do not send a link. Do not pitch. Do not ask why.
- Do not try one more angle, and do not ask for a referral to someone else.
- Two lines. Acknowledge it plainly, leave one door open, stop.
- "No problem, thanks for the straight answer. If that changes, I am around." is
  a complete and good reply. Nothing needs adding to it.
- Someone who declines politely and gets a pushy reply tells other people.
...
[the times, if any were given: each on its own line, exactly as provided]
[one short catch-all line with the booking link, if one was given]
...
export async function generateReply({ category, replyText, leadName, subject, leadEmail, baseUrl, senderName, conversationId, leadTimezone }) {
  ...
  // A decline gets no link. Fetching one burns a single-use Calendly link on
  // someone who said no, and its presence in the context is what made the model
  // offer times to people who had just turned the meeting down.
  const cat = String(category || "").toLowerCase();
  const declined = cat.includes("not interested") || cat.includes("decline") || cat.includes("no thanks");
  const link = declined
    ? null
    : await safeBookingLink({ name: leadName || "", email: leadEmail || "" });

  // Naming two or three real times converts better than a bare link, and each
  // one books that exact slot, so the lead picks a time instead of a website.
  const times = declined
    ? []
    : await safeAvailableTimes({ count: 3, timezone: leadTimezone || "" });
  ...
  const ctx = [
    ...
    declined
      ? "THEY ARE DECLINING. Do not offer a call, a link, or times. Do not pitch. Two lines: acknowledge it, leave the door open once, stop."
      : times.length
        ? "Times to offer. Reproduce these EXACTLY, one per line, each as a markdown link, in this order. Do not reword them, do not invent others, do not convert the timezone:\n"
          + times.map((t) => `- [${t.label}](${t.url})`).join("\n")
          + (link ? `\nThen one short line offering the catch-all: ${link}` : "")
      : link
