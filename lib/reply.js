...
- Always write in English, whatever language the lead used. Never reply in another language.
...
const BUMP_SYSTEM = `You write a short follow-up nudge to a lead who showed interest, got a reply from us, and then went quiet.

Rules:
- Always write in English, whatever language the lead used. Never reply in another language.
- Very short. Two or three lines, under 45 words excluding any times.
- Warm and low pressure. Assume the email got buried, never imply they were rude or slow.
- Do not re-pitch. They already know what this is.
- If times are provided, offer them plainly as "• " bullets, verbatim.
- If this is the final nudge, say so lightly and make it easy to close the loop ("if the timing is off just say so and I'll leave it there").
- Everyday English. No em dashes, no semicolons, no emojis, no exclamation marks.
- Banned: leverage, streamline, circle back, touch base, "just checking in", "following up on my last email", "I hope this email finds you well".
- Sign with the sender's first name if given.
...
export async function generateBump({ leadName, senderName, said, bumpNumber = 1, isFinal = false, leadEmail, baseUrl, leadTimezone }) {
  ...
  let picked = [], slotLines = [];
  try {
    picked = pickSlots(await getAvailableSlots(), 2, leadTimezone || process.env.TIMEZONE || "America/New_York");
    slotLines = picked.map((s) => s.label);
  } catch { /* a nudge without times is still fine */ }
  ...
  const ctx = [
    ...
    slotLines.length ? `Times you may offer (verbatim):\n${slotLines.join("\n")}` : "No times available, do not invent any.",
  ].filter(Boolean).join("\n\n");
  ...
  const html = renderReplyHtml(body, { slots: picked, leadEmail, leadName, baseUrl });
  return { body, html, slots: picked.map((s) => ({ start_time: s.start_time, end_time: s.end_time, label: s.label })) };
}
