...
// The sender persona for a conversation = the mailbox the lead replied to,
// e.g. "stephanie.taylor@koldmailworks.com" -> "Stephanie". Signing with the
// right persona keeps the thread coherent.
export function senderPersona(messages) {
  const inbound = latestInbound(messages, { skipAutoReplies: false });
  const to = String(inbound?.to || "").toLowerCase();
...
// Signals that a human should look at this conversation before the agent
// keeps automating it.
const REVIEW_RULES = [
  { flag: "Doesn't want a call", re: /\b(no|not|don'?t|do not|rather not|without)\b[^.!?]{0,30}\b(call|meeting|zoom|demo)\b|\b(call|meeting)\b[^.!?]{0,20}\bnot (necessary|needed)\b/i },
  { flag: "Suspects AI", re: /\b(ai|bot|automated|chatgpt|generated)\b[^.!?]{0,25}\b(reply|response|email|message)\b|\bis this (a )?(bot|ai|human|real)\b/i },
  { flag: "Asked a direct question", re: /\?\s*$|\b(what|how much|how many|which|who|when|why|can you|do you|does it|is it)\b[^.!?]{0,80}\?/i },
  { flag: "Wants another channel", re: /\b(whatsapp|phone|text|sms|linkedin|call me on)\b/i },
  { flag: "Mentions pricing", re: /\b(price|pricing|cost|per lead|budget|quote|how much)\b|\$\s?\d/i },
];

export function reviewFlags(text) {
  const own = String(text || "")
    .split(/\bFrom:\s/i)[0]
    .split(/\bOn\s[\s\S]{5,220}?wrote:/i)[0]
    .split(/\n\s*>/)[0];
  return REVIEW_RULES.filter((r) => r.re.test(own)).map((r) => r.flag);
}
...
export function isOptOut(subject, bodyText) {
  const subj = String(subject || "").replace(/^(re|fwd?):\s*/gi, "");
  // Only scan the lead's own words, not the quoted original below their reply.
  // Quotes start with "From: ..." (Outlook) or "On <date> ... wrote:" (Gmail)
  // or "> " prefixed lines.
  const ownText = String(bodyText || "")
    .split(/\bFrom:\s/i)[0]
    .split(/\bOn\s[\s\S]{5,220}?wrote:/i)[0]
    .split(/\n\s*>/)[0];
