...
export function signSlot(email, startTime, durationMin) {
  return crypto
    .createHmac("sha256", secret())
    .update(`${email}|${startTime}|${durationMin}`)
    .digest("hex")
    .slice(0, 24);
}

export function verifySlot(email, startTime, durationMin, sig) {
  try {
    const expected = signSlot(email, startTime, durationMin);
...
export function bookUrl(baseUrl, { email, name, startTime, durationMin }) {
  const p = new URLSearchParams({
    e: email,
    t: String(startTime),
    d: String(durationMin),
    ...(name ? { n: name } : {}),
    sig: signSlot(email, startTime, durationMin),
  });
