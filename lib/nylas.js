...
async function nylas(method, path, body) {
...
// natural "here are a few times" email), formatted for the reply.
export function pickAndFormatSlots(timeSlots, count = 3, tz = process.env.TIMEZONE || "America/New_York") {
  ...
  return picked.map((s) => formatSlot(s.start_time, tz));
}
